...
Protocol bearer leases are transport-only authorization. Mission Control persists a strict `agent-protocol-receipt/2` acknowledgement containing a response checksum and, when a lease was issued, only a lease ID, SHA-256 token fingerprint, issue/expiry times, fence, and exact workspace/agent/credential/assignment/resource binding. Unknown fields and secret-like fields are rejected at every receipt nesting level in both the application schema and database constraint. Mission Agent applies the same rule to local state: active assignment metadata is durable, but `leaseToken` is replaced with a non-secret authorization receipt before writing. A restart cannot recover bearer authority from disk and must wait for or obtain a freshly fenced lease.
...
## Consensus Plan boundary

`consensus_plan` is a planning-only specialization of the existing Mission → Task → Execution machinery. A consensus aggregate projects the bounded phase state, two role assignments, immutable repository and Project Brain bindings, artifact graph, objections, exact-plan verdicts, human approval, and one child implementation reference. Each planner turn is still a normal task and signed remote execution; the consensus coordinator only decides which bounded task becomes eligible next.

The server withholds both proposal bodies until two valid independent proposal artifacts are durable. It then releases only the opposite proposal to each critique turn, releases each planner's own proposal plus its received critique to revision, and gives the configured synthesizer the exact six-artifact package. The canonical plan is content-hashed once and cannot be replaced after verdict collection begins. Consensus requires two independent verdicts against that hash and zero open blockers; textual similarity is never an authority signal.

Human approval binds the deterministic child, repository snapshot/base branch/base commit, Project Brain context hash, canonical artifact/hash, exact executor agent/provider/model/assignment/attestation, permission profile, fixed execution budget, exact owner-governed validation commands, allowed local effects, and permanent prohibitions. Approval creates no writes by itself and is the only child mutation grant; no second generic approval is requested. A unique Repository Change Mission is created only after approval and repository-staleness checks. The selected executor acknowledges the exact plan, agent, provider, model, snapshot, context, and permission profile before mutation. Success requires complete artifacts and an immutable authenticated validation receipt; crash recovery otherwise pauses for review. On child completion, Mission Control proposes an evidence-only Project Brain learning candidate; it never promotes that candidate automatically.

Agent identity, provider, runtime model, consensus role, and phase are separate dimensions. Each role assignment binds an explicit advertised model capability and current immutable attestation. Current Codex and Claude Code adapters pass the selected model via exact `--model` arguments, but installed tools cannot independently prove the actual runtime model for every invocation; those capabilities are therefore explicitly `unverifiable`, and any detectable fallback mismatch is rejected. There is no automatic model routing in this release.

Provider execution requirements are also a separate, versioned authority dimension. The provider catalog declares the executable/authentication probe, host platform, isolation/network/temp/credential boundaries, repository access, model-selection and fallback behavior, structured-output mechanism, diagnostic policy, and consensus eligibility for `codex`, `claude_code`, `hermes`, `generic`, and `mock`. Mission Agent reports a hash-bound readiness status on each authenticated heartbeat. The server fixes that contract at owner-approved registration, requires it for capability eligibility, and copies the ID/hash into each participant assignment. A catalog change therefore invalidates prior readiness and requires a new Mission Agent artifact approval; it cannot silently broaden provider access or substitute a runtime.

The superseding development proposal adds four operation-specific macOS profiles. All assignment and provider roots are canonicalized. Codex receives an assignment-private read-only symlink to the exact `auth.json` rather than copied credential bytes; Claude receives only a brokered OAuth token selected by exact service, account, and keychain bindings and has no in-sandbox Keychain or real-HOME access. External TCP/UDP and the exact mDNS resolver socket are allowed for provider inference; loopback, inbound, bind, and other Unix sockets are denied. Codex implementation uses Mission Agent's outer Seatbelt profile as the sole write authority because nested Seatbelt prevents its edit helper from functioning; the shell tool remains disabled and `.git` writes remain denied. Exact PATH launcher bytes/path, exact invoked executable bytes/path, resolved runtime roots, sandbox-template bytes, and credential-broker identities are bound into heartbeat attestations and assignments and revalidated before credential access. Codex bypasses the JavaScript launcher and directly invokes its hash-bound native binary so the actual executed provider code is part of the binding.

Disposable consensus creation additionally requires a clean `complete_repository_state/3` binding covering canonical repository identity, checkout-root identity, base branch/commit, HEAD, tracked status, complete independently hashed tracked worktree bytes/modes/symlink targets compared with the index, complete content-addressed untracked and policy-relevant ignored manifests, and recursive submodule state. Authenticated registration stores that exact state in the repository projection and a separately identified immutable snapshot row whose checksum is the state hash. The registration event binds the authenticated message, credential, request-body checksum, and bounded receipt authority; replay recreates the same state and snapshot without inspecting the filesystem. Planning and implementation re-hash the live checkout and reject drift before provider invocation. An initial planner terminal failure appends the terminal consensus fact and fences both planner outputs transactionally before sibling cancellation is requested; artifact append revalidates the terminal status and assignment fence while holding the consensus aggregate lock.

Disposable implementation authority is a separate, additive `repository-authority/1` binding. An authenticated owner command appends `repository.authority_configured` and stores an immutable receipt binding the owner, command, event, prior authority hash, and new canonical authority hash. The only accepted disposable profile permits repository inspection, isolated-worktree mutation by named implementation agents, and a Mission Agent-controlled local commit. It permanently denies provider-direct commit, push, pull-request creation, publication, deployment, and infrastructure mutation. The command also narrows per-agent repository permissions; the acceptance harness cannot patch repository projections directly.

The authority binding is revalidated at mission creation, planning dispatch/claim/renewal, approval, child creation, executor dispatch/claim/renewal, mutation, local commit, and terminal success. The command identity participates in the binding hash, so an authority change followed by a byte-identical restoration cannot revive an old lease or approval. Legacy repository flags remain available for existing non-consensus workflows through an explicit compatibility projection; no existing production permission is revoked by the additive migration, and the stricter profile is required only for the disposable consensus path.

`APP_ENV=disposable_acceptance` is an explicit non-production trust mode and is never inferred from localhost or `NODE_ENV`. Startup requires an exact non-writable local registry path and content hash, a loopback disposable database with an operator-declared name, loopback HTTP endpoints, and local artifact storage inside the disposable root. Production credentials, production registry authority, S3 configuration, and provider-writable registry placement fail startup. Production mode rejects every disposable registry setting. Runtime mode, registry path/hash, scope, and expiry are exposed in readiness and bound into artifact events, capability attestations, eligibility, and protocol receipts. The UI and structured logs label this mode `DISPOSABLE ACCEPTANCE — NON-PRODUCTION`; it has no signing, publication, or deployment authority.

See `docs/CONSENSUS_PLAN_AND_CLAUDE_CODE_IMPLEMENTATION.md` and `docs/CONSENSUS_PLAN_OPERATOR_RUNBOOK.md`.
