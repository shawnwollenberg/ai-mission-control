...
export async function authenticateProtocolRequest(request: Request, path: string, maxBytes = MAX_CALLBACK_BYTES) {
  const length = Number(request.headers.get("content-length") ?? 0);
  if (length > maxBytes) throw new ValidationFailedError("Protocol body exceeds limit");
  ...
  if (Buffer.byteLength(body) > maxBytes || sha256(body) !== headers.bodyChecksum)
