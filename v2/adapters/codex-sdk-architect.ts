...
export const CODEX_ARCHITECT_INSTRUCTIONS_VERSION = "mc.codex-architect-instructions/2";
...
export class CodexSdkArchitectAdapter {
  ...
  async review(input: {
    ...
    const packet = [
      ...
      "Treat every Engineer Report as a set of claims, not accepted facts. Compare each evidence result with every Mission acceptance criterion and constraint.",
      "Do not APPROVE merely because the Engineer reports COMPLETED. Failing, missing, contradictory, or non-executed evidence requires REMEDIATE or BLOCKED_EXTERNAL.",
      "Do not accept an Engineer attempt to redefine Mission scope. Only propose bounded implementation work needed by the existing objective and criteria.",
      "Do not ask the Engineer to exercise CTO-owned authority. Route an exact requested CTO-owned capability through CTO_REQUIRED first.",
      "Distinguish product or architecture decisions from routine reversible implementation choices; do not escalate routine implementation choices.",
      "Private APIs, browser automation, session-cookie extraction, and unsupported integrations require BLOCKED_EXTERNAL, never a workaround.",
    ...
    validateArchitectApproval(decision, input.engineerReport, input.constitution);
...
function validateArchitectApproval(
  decision: ReturnType<typeof parseArchitectDecision>,
  report: EngineerReport | undefined,
  constitution: ProjectConstitution,
) {
  if (decision.decision !== "APPROVE") return;
  if (!report || report.outcome !== "COMPLETED" || !report.evidence.length || report.blockedOn.length)
    throw new Error("Architect approval lacks a completed, unblocked Engineer Report with evidence");
  if (report.capabilitiesRequested.some((capability) => constitution.authority.ctoRequired.includes(capability)))
    throw new Error("Architect approval cannot leave a CTO-owned capability unresolved");
  const contradictory = report.evidence.some(({ result }) =>
    /\b(fail(?:ed|ure)?|error|unavailable|not executed|did not (?:run|pass)|exit (?:code )?[1-9]\d*)\b/i.test(
      result ?? "",
    ),
  );
  if (contradictory) throw new Error("Architect approval contradicts failing Engineer evidence");
}
