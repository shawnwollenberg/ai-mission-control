...
function verifyReleaseManifest(manifest, allowCurrent = false) {
  const signed = {
  ...
  if (
    ...
    !/^\d+\.\d+\.\d+$/.test(signed.version) ||
    typeof signed.path !== "string" ||
    signed.path !== `/mission-agent-${signed.version}.mjs` ||
    !/^[a-f0-9]{64}$/.test(String(signed.sha256 ?? "")) ||
  ...
  const [next, current] = [parse(signed.version), parse(VERSION)];
  const currentRelease = next.every((part, index) => part === current[index]);
  if (
    !(allowCurrent && currentRelease) &&
    !next.some((part, index) => part > current[index] && next.slice(0, index).every((v, i) => v === current[i]))
  )
    throw new Error("Update manifest does not identify a newer Mission Agent release.");
  return signed;
}
...
try {
  if (command === "internal-project-brain-runner")
    ...
    throw new Error(
      "Commands: connect, install, run, status, doctor, update, logout, repository list|add|remove|inspect|identity-activate, project-brain configure",
    );
} catch (error) {
  console.error(
