...
async function executeAnalysis(config, assignment) {
  ...
  const recommendationPrompt = `Inspect this repository in read-only mode for the analysis objective: ${assignment.instructions ?? assignment.taskObjective}. Return ONLY a valid JSON array containing 1 to 8 focused recommendations. Each object must have: title, description, reasoning, evidence (array of objects with repository-relative path, optional positive line, and description), estimatedImpact (low|medium|high|critical), estimatedRisk (low|medium|high), estimatedEffort, suggestedValidation (array of safe commands using npm, pnpm, yarn, bun, npx, node, go, cargo, or pytest), and acceptanceCriteria (array of observable outcomes). Every recommendation must cite visible file evidence. Do not modify files.`;
  const recommendationResult = await runCodex(config, assignment, ["exec", "--sandbox", "read-only", "--skip-git-repo-check", "-o", recommendationsPath, recommendationPrompt], resolved);
  if (recommendationResult.exitCode !== 0) throw new Error(`Codex recommendation extraction failed${recommendationResult.stderr ? ": " + recommendationResult.stderr.slice(-300) : "."}`);
  const recommendationBody = await readFile(recommendationsPath);
  let recommendationValue;
  try { recommendationValue = JSON.parse(recommendationBody.toString("utf8")); } catch { throw new Error("Codex returned invalid structured recommendations."); }
  ...
  if (exec("git", ["status", "--porcelain=v1", "--untracked-files=all"], resolved) !== before || exec("git", ["rev-parse", "HEAD"], resolved) !== beforeCommit)
    throw new Error("Read-only recommendation verification detected a repository change.");
  await uploadArtifact(config, assignment, { name: "Repository recommendations", description: "Structured evidence-backed recommendations from repository analysis", type: "repository_recommendations", mediaType: "application/json", body: recommendationBody, repositoryCommit: beforeCommit });
...
function safeValidationCommands(value) {
  ...
  return value.map((command) => {
    ...
    if (command.some((part) => typeof part !== "string" || !/^[A-Za-z0-9_./:@=,+-]+$/.test(part) || (part.includes("..") && part !== "./...")))
