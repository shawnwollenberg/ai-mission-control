...
async function executeAnalysis(config, assignment) {
  ...
  const recommendationPrompt = `Inspect this repository in read-only mode for the analysis objective: ${assignment.instructions ?? assignment.taskObjective}. Return ONLY one valid JSON object with two properties: recommendations and observations. recommendations must be an array containing 1 to 8 focused objects with title, description, reasoning, evidence (repository-relative path, optional positive line, description), estimatedImpact (low|medium|high|critical), estimatedRisk (low|medium|high), estimatedEffort, suggestedValidation (safe npm, pnpm, yarn, bun, npx, node, go, cargo, or pytest commands), and acceptanceCriteria. observations must cover each dimension architecture, tests, security, technical_debt, documentation, dependencies, and ci at least once. Each observation has dimension, status (strength|risk|unknown), severity (low|medium|high|critical), summary, and evidence. A strength or risk requires visible repository-relative file evidence; use unknown with empty evidence when the repository cannot support a claim. Do not infer health from missing files and do not modify files.`;
...
async function executeChange(config, assignment) {
  ...
  const patch = spawnSync("git", ["diff", "--binary", "HEAD"], {
    cwd: worktreePath,
    encoding: "utf8",
    timeout: 60_000,
  });
...
async function run() {
  ...
  for (;;) {
    try {
      ...
      if (publicationResponse?.publication) await publishForReview(config, publicationResponse.publication);
