...
async function registerRepository(config, path) {
  ...
  config.repositories = {
    ...(config.repositories ?? {}),
    [registered.repository_id]: { path: repository.path, fingerprint: repository.fingerprint, name: repository.name },
  };
...
try {
  ...
  else if (command === "repository" && process.argv[3] === "add")
    console.log(await registerRepository(await loadConfig(), process.argv[4]));
  ...
  } else throw new Error("Commands: connect, run, status, doctor, logout, repository add");
} catch (error) {
  console.error(
