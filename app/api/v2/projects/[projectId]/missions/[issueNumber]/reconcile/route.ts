...
export async function POST(
  ...
  try {
    ...
    if (!body.reason?.trim() || !Array.isArray(body.evidence) || !body.evidence.length)
      throw new Error("A reconciliation reason and evidence are required");
    const runtime = await createV2MissionRuntime(projectId);
    const result = await runtime.orchestrator.reconcileExternalBlock(issueNumber, body);
    ...
    return NextResponse.json(
      { error: { code: "owner_reconciliation_failed", message: (error as Error).message } },
      { status: 409 },
    );
  }
}
