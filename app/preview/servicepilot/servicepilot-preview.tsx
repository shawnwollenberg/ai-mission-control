...
export default function ServicePilotPreview() {
  ...
  return (
    <main className="servicepilot-shell">
      <nav className="servicepilot-nav">
        <strong>ServicePilot</strong>
        <span>Controlled preview · no live charges</span>
      </nav>
      ...
      <section className="pricing-grid">
        {SERVICEPILOT_PLANS.map((plan) => (
          ...
          >
            ...
            <p>{plan.detail}</p>
            <b>{selectedPlan === plan.name ? "Selected" : "Select plan"}</b>
          </button>
        ))}
      </section>
      <section className="checkout-panel">
        ...
        <button onClick={() => setCheckoutReady(true)}>
          Start subscription <span>→</span>
        </button>
      </section>
      {checkoutReady && (
        <section className="checkout-confirmation" aria-live="polite">
          <span>✓</span>
          <div>
            <strong>Checkout flow verified</strong>
            <p>{createCheckoutMessage(selectedPlan)}</p>
          </div>
        </section>
      )}
    </main>
