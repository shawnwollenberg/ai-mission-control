...
export async function processOneOutbox(workerId: string, workspaceId?: string) {
  const message = await withTransaction(async (client) => {
    ...
    const r = await client.query<Outbox>(
      `SELECT id::text,workspace_id,event_id,topic,payload,attempt_count,correlation_id
       FROM outbox WHERE status IN('pending','failed') AND available_at<=now()
